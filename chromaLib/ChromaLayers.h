...
{
    if (err.data.size() == 0) return Eigen::VectorXf();
    if (err.data.rows() == 1)
...
public:
    convolutionalLayer (int /*inputChannels*/, int /*outputChannels*/, int kernelSize)
    {
        ...
        // IMPORTANT: don’t start as all-zero, or you’ll get “stuck 0”
        // tiny symmetric init (deterministic-ish)
    ...
    FeatureTensor forwardconvolve (const FeatureTensor& input)
    {
        ...
        if (K <= 0 || input.data.size() == 0) return input;

        // expects Nx1
        const int inN = (int)input.data.rows();
        if (input.data.cols() < 1 || inN <= 0) return input;
